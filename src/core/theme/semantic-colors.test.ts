...
const testGoodContrast = (
  ...
  it(`${colorPath}(${color}) has enough contrast with ${backgroundPath}(${background}) to be ${standart}`, () => {
    expect(getContrast(color, background)).toBeGreaterThanOrEqual(
      guidelines[standart]
    )
  })
}
...
describe('semanticColors', () => {
  ;['light', 'dark'].forEach((mode) => {
    describe(mode, () => {
      ...
      ;['default', 'content1', 'content2', 'content3', 'content4'].forEach(
        (name) => {
          testGoodContrast(
            `${mode}.foreground.DEFAULT`,
            `${mode}.${name}.DEFAULT`,
      ...
      ;['primary', 'secondary', 'success', 'warning', 'danger'].forEach(
        (name) => {
          testGoodContrast(
            `${mode}.${name}.foreground`,
            `${mode}.${name}.DEFAULT`,
